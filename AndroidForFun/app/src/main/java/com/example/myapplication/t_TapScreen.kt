...
class t_TapScreen {
    ...
    fun renew(ev: MotionEvent): Double {
        ...
        if (ev.actionIndex == FirstTouch.pointer) {
            if (!FirstTouch.isMoving and
                (abs(FirstTouch.Begin.x - ev.x) <= 10.0) and (abs(FirstTouch.Begin.y - ev.y) <= 10.0))
                return (res)
            ...
            if (!SecondTouch.isMoving and
                (abs(SecondTouch.Begin.x - ev.x) <= 10.0) and (abs(SecondTouch.Begin.y - ev.y) <= 10.0))
                return (res)
